(() => {
  ...
  const KEY = "huanyuanliu.expanded.v2";
  const VIEWS = ["home", "forum", "topic", "lostfound", "phone", "diary", "gazette", "codex", "ledger", "contract", "drawer", "keeper", "evidence", "profile", "news", "mail", "map"];
  ...
  try {
    ...
    if (data && data.version === 1) {
      ...
      state.newsReads = ["short", "busco", "custom"].filter((key) => Array.isArray(data.newsReads) && data.newsReads.includes(key));
      state.mapLayer = ["admin", "busline", "found"].includes(data.mapLayer) ? data.mapLayer : "found";
  ...
  function render(scroll = false) {
    ...
    else main.innerHTML = { home: renderHome, forum: renderForum, topic: renderTopic, lostfound: renderLostFound, phone: renderPhone, diary: renderDiary, gazette: renderGazette, codex: renderCodex, ledger: renderLedger, contract: renderContract, drawer: renderDrawer, keeper: renderKeeper, evidence: renderEvidence, profile: renderProfile, news: renderNews, mail: renderMail, map: renderMap }[state.view]();
    icons();
    if (scroll) {
  ...
  function topicInfo(key) {
    if (key === "wish") return { category: "乡土民俗", title: "【愿望柳】留一句心愿，等一阵好风", author: "守柳人", date: "今天 23:02", replies: "3,288", sticky: true };
  ...
  function renderForum() {
    ...
    const replyCount = (3288 + state.forumReplies.filter((reply) => reply.rootId.startsWith("wish-")).length).toLocaleString("en-US");
  ...
  function renderPost(post, id) {
    ...
    if (has("ledger") && id === "wish-found-0")
      body = '<p><strong>[ 该用户不存在 ]</strong></p><p>愿一个重要的人过得好。</p><p>愿望灵验之后，我会回来还愿。</p><div class="receipt-strip">自留凭单　<strong>LX0617</strong>　<span>待还</span></div><p class="deleted-line">[ 本帖经系统整理于 23:41 ]</p><p class="lantern-fragment residue">六月那盏灯，我挂在北门桥上，没挂进树洞。</p>';
    ...
    let lanternBlock = "";
    if (id === "wish-found-0" && has("homeDate")) {
      lanternBlock = has("lantern")
        ? `<div class="lantern-hunt kept"><p>这张红纸已经揭下来了。${has("ledger") ? "名字被系统整理掉了，这行字却还在——它不是写给柳的。" : "桥上的灯，归你保管。"}</p></div>`
        : `<div class="lantern-hunt"><p>末尾这行字和上面的语气不太一样，像是从别处抄进来，特意留给看得懂的人。${has("ledger") ? "名字都被整理掉了，它却还在。" : ""}</p><button class="command secondary" data-action="keep-lantern">${icon("flame")}把这张红纸揭下来</button></div>`;
    }
    return `<article class="post ${post.tone || ""}" data-post-id="${escape(id)}"><div class="post-author"><span class="avatar">${post.avatar}</span><strong>${post.name}</strong><small>${post.role}</small>${post.tone === "keeper" ? '<span class="role-tag">版主</span>' : ""}</div><div class="post-content"><div class="post-meta"><span>发表于 ${post.date}</span><span>${post.floor}</span></div><div class="post-body">${body}</div>${lanternBlock}${postActions(meta)}</div></article>${renderForumReplies(id)}`;
  }
  ...
  function renderContract() {
    ...
    const fragments = [
      ["accident", "晚报剪报", "守柳人的剪报册里，有那个月的晚报。"],
      ["borrow", "图书馆代签", "图书馆还挂着他一笔没还的旧账。"],
      ["eastGate", "东环路拾遗", "县地图服务的拾遗图层，登过一件外套。"],
      ["lantern", "桥上的灯", "回到第 3241 楼，再读一遍他的愿望。"],
    ];
    const fragMissing = fragments.filter(([key]) => !has(key));
    const outsideReady = has("witness") && fragMissing.length === 0;
    const fragList = `<ul class="fragment-list">${fragments.map(([key, label, hint]) => `<li class="${has(key) ? "done" : ""}">${icon(has(key) ? "check" : "circle")}<div><strong>${label}</strong>${has(key) ? "" : `<small>${hint}</small>`}</div></li>`).join("")}</ul>`;
  ...
  function renderKeeper() {
    ...
    return `${heading("还愿登记 / 2009 年残页", "一条被涂改的记录。", "所有登记里，只有这一条，名字被反复涂黑，最后写着「佚」。")}
      ...
      ${has("keeper") ? `<div class="block-action"><p>他的柜子里还有一册剪报，按月份码放，唯独 2026 年 6 月那一格塞得鼓鼓的。</p><button class="command secondary" data-action="open-news">${icon("newspaper")}翻看那册剪报</button></div>` : ""}`;
  }
  function renderNews() {
    const read = (key) => state.newsReads.includes(key);
    const allRead = ["short", "busco", "custom"].every(read);
    const clip = (key, date, page, title, body) => `
      <article class="news-clip ${read(key) ? "read" : ""}"><header><time>${date}</time><span>${page}</span>${read(key) ? `<b>${icon("check")}已读</b>` : ""}</header><h3>${title}</h3>${read(key) ? `<div class="news-body">${body}</div>` : `<button class="command secondary" data-action="read-news" data-clip="${key}">${icon("book-open")}展开这条剪报</button>`}</article>`;
    const audit = allRead
      ? has("accident")
        ? `<div class="news-verified"><span>${icon("badge-check")}</span><div><strong>那晚已经拼回去了：6 月 17 日 23:40，北岸河堤。</strong><p>灯上写着「还愿」。他落水前，已经完成了一次「送」。</p></div></div>`
        : `<form class="news-audit" id="news-audit-form"><header><span>${icon("calendar-search")}</span><div><h3>把那一晚拼回去</h3><p>三条剪报放在一起，时间、地点对得上吗？</p></div></header><div class="timeline-fields"><label>他最后被看见<select id="news-date" required><option value="">请选择</option><option value="0616">6 月 16 日</option><option value="0617">6 月 17 日</option><option value="0618">6 月 18 日</option></select></label><label>大约是<select id="news-time" required><option value="">请选择</option><option value="2247">22:47</option><option value="2340">23:40</option><option value="0016">00:16</option></select></label><label>地点在<select id="news-place" required><option value="">请选择</option><option value="cave">柳祠树洞</option><option value="bank">北岸河堤</option><option value="bridge">北门桥头</option></select></label></div><p class="inline-error" id="news-error" role="status"></p><button class="command">${icon("combine")}核对剪报</button></form>`
      : "";
    return `${heading("本地出版 / 青河晚报", "电子报 · 2026 年 6 月剪报", "守柳人的剪报册。他留下了每一张提到那个月河边的报纸。")}
      <section class="news-shelf">${clip("short", "2026-06-18", "第 2 版 · 社会", "北岸河堤昨夜有人落水，搜救暂无结果", "<p>本报讯　昨晚 23 时 40 分许，有夜钓者报警称，青河北岸河堤（北门桥下游约三百米）有人落水。公安、消防到场搜救至今晨，暂未发现落水者，现场亦无随身物品。</p><p>警方提醒：近期连降夜雨，河堤湿滑，请勿夜间靠近。</p>")}${clip("busco", "2026-06-18", "第 4 版 · 公告", "关于 6 月 17 日夜间运营情况的说明", "<p>针对有群众询问 6 月 17 日晚北门桥一带情况，青河公交公司声明：当日 3 路、7 路末班车均准点发出、正常收班，无甩站、无中途停车，车载记录完整。</p><p>请勿轻信与传播不实信息。</p><p class='quiet-line'>没有人问过末班车。他们自己急着说的。</p>")}${clip("custom", "2026-06-22", "第 7 版 · 风物", "六月放灯", "<p>青河旧俗，六月放灯。灯不入祠，不系柳。老人说：挂在柳上的，是求；挂在别处的，是留。求的要还，留的，不必还。</p><p>近年放灯者渐少。本月十七日夜，有夜归人见北门桥桥栏上挂着一盏孤灯，灯上两个字：林舟。风吹了一夜，那灯没有掉。</p>")}</section>
      ${audit}
      <div class="block-action"><p>${allRead ? "报纸不属于论坛，也不属于登记处。它们归档案馆管。" : "剪报要一张一张看完，再谈核对。"}</p><button class="command secondary" data-action="open-keeper">${icon("arrow-left")}放回柜子</button></div>`;
  }
  function renderMail() {
    if (!has("borrow"))
      return `${heading("公共服务 / 青河县图书馆", "馆际互借与代签查询", "本系统与户政底册联网。读者证号即凭单号。")}
      <div class="mail-gate"><div class="ledger-seal">${icon("library")}</div><h3>读者查询</h3><p>可查在借、逾期与代签记录。<br>仅向持证人或有据近亲开放。</p><form id="mail-form"><div class="form-field"><label for="mail-card">读者证号</label><input id="mail-card" placeholder="与凭单同号" autocomplete="off" maxlength="16" required></div><p class="inline-error" id="mail-error" role="status"></p><button class="command">${icon("book-key")}查询借阅记录</button></form><p class="access-footnote">逾期不还的书，图书馆会一直等。</p></div>`;
    return `${heading("公共服务 / 青河县图书馆", "馆际互借记录 · LX0617", "查询时间：刚刚 · 该证号状态：在借 1 册，代签 1 件")}
      <section class="mail-result"><header><h3>借阅记录</h3><span>2 条</span></header><ol><li><div><strong>《青河县水利志（民国稿抄本）》</strong><small>借出 2026-06-10 · 应还 2026-06-24</small></div><span class="mail-flag overdue">逾期未还</span></li><li><div><strong>《柳祠香会簿》光绪二年抄本（影像）</strong><small>借出 2026-06-15 · 当日归还</small></div><span class="mail-flag">已归还</span></li></ol></section>
      <section class="mail-result"><header><h3>代签记录</h3><span>1 条</span></header><ol><li><div><strong>林舟 → 林家 · 平信一件</strong><small>2026-06-18 退至北桥邮局 · 由「北桥老周」代收 · 未录入内件</small></div><span class="mail-flag signed">已代收</span></li></ol><p class="mail-note">他借了水利志，没有还。他寄了一封信回家，没走柳巷的邮局。</p></section>
      <div class="block-action"><p>图书馆的系统，论坛改不到。这条逾期记录会一直挂在那里，等人来续借。</p><button class="command secondary" data-action="home">${icon("arrow-left")}返回论坛</button></div>`;
  }
  function renderMap() {
    const layers = [["admin", "map", "行政区划"], ["busline", "route", "公交线路"], ["found", "package-search", "拾遗登记"]];
    const panel = {
      admin: `<div class="map-list"><h3>北门桥一带 · 地名</h3><ol><li>北门桥（青河北岸）</li><li>东环路（北门桥下游，沿河）</li><li>北岸河堤（东环路再往下游约两百米）</li><li>柳巷（县城西南，柳祠所在）</li></ol><p class="map-note">从北门桥顺水而下，先到东环路，再到北岸河堤。</p></div>`,
      busline: `<div class="map-list"><h3>3 路 / 7 路 · 途经站点（节选）</h3><ol><li>老汽车站</li><li>文化路北口</li><li>供销社</li><li>北桥东侧</li><li>北门桥</li><li>东环路（招呼站）</li></ol><p class="map-note">6 月 17 日夜间运营记录：准点，无甩站。——他们自己声明的。</p></div>`,
      found: `<div class="map-list"><h3>拾遗登记 · 6 月 17 日至 18 日</h3><ol><li><div><strong>拾遗 015 · 公交卡一张</strong><small>6 月 17 日 21:02 · 北桥公交站 · 待认领</small></div></li><li><div><strong>拾遗 016 · 雨伞一把</strong><small>6 月 18 日 05:50 · 老汽车站 · 待认领</small></div></li><li class="key-item"><div><strong>拾遗 017 · 深蓝色连帽外套一件</strong><small>6 月 18 日 06:20 · 东环路（北门桥下游）· 存清水巷临时管理房</small></div><span class="mail-flag overdue">无人认领</span></li></ol><p class="map-note">公交卡在北桥站，外套在东环路，人最后到了河堤——一路顺水往下游。</p></div>`,
    }[state.mapLayer];
    const question = has("eastGate")
      ? `<div class="news-verified"><span>${icon("badge-check")}</span><div><strong>路线已经连起来了：北门桥 → 东环路 → 北岸河堤。</strong><p>他落水前，还往下游走了一段。他不是失足，是走到水边去的。</p></div></div>`
      : `<form class="news-audit" id="map-form"><header><span>${icon("waypoints")}</span><div><h3>把三个位置连起来</h3><p>北门桥 → ？ → 北岸河堤。走向河边的路上，他把外套留在了哪一段？</p></div></header><div class="timeline-fields"><label>外套掉在哪一段<select id="map-place" required><option value="">请选择</option><option value="station">老汽车站</option><option value="east">东环路</option><option value="willow">柳祠</option></select></label></div><p class="inline-error" id="map-error" role="status"></p><button class="command">${icon("combine")}连起这条路线</button></form>`;
    return `${heading("公共服务 / 青河县地图服务", "地图与便民图层", "数据来源：县自然资源局 · 拾遗数据每日同步")}
      <div class="map-toolbar">${layers.map(([key, ic, label]) => `<button class="map-chip ${state.mapLayer === key ? "active" : ""}" data-action="map-layer" data-layer="${key}">${icon(ic)}${label}</button>`).join("")}</div>
      ${panel}
      ${question}
      <div class="block-action"><p>图层是县里维护的。论坛管不到地图。</p><button class="command secondary" data-action="home">${icon("arrow-left")}返回论坛</button></div>`;
  }
  ...
  function renderProfile() {
    ...
    return `${crumb("用户中心 / 晚来风")}
      ${searchResult}<section class="profile-page">
        ...
        <section class="profile-history"><header><div><h3>历史发言</h3><p>按时间倒序：她本人的发言，以及提到她的旧帖。</p></div><span>3 条</span></header><ol><li><time>09-12</time><button data-action="open-topic" data-topic="photos"><strong>征集青河老街、柳巷和旧码头照片</strong><small>我家有几张 2014 年河边的照片，周末找找。</small></button><span>旧城相册</span></li><li class="key-record"><time>06-16</time><button data-action="open-topic" data-topic="waterJune"><strong>柳巷西段晚间临时停水</strong><small>一叶小舟提到你：刚从北桥把我姐接回家。</small></button><span>${readWater ? "已读" : "生活互助"}</span></li><li class="key-record"><time>06-16</time><button data-action="open-topic" data-topic="returnTrip"><strong>晚班车临时改停北桥</strong><small>晚来风：刚从外地回来，今晚能回柳巷。</small></button><span>${readTrip ? "已读" : "青河杂谈"}</span></li></ol></section>
        ${timeline}${trace}
  ...
  function choose(ending) {
    ...
    if (ending === "recorded" && (!has("witness") || !has("accident") || !has("borrow") || !has("eastGate") || !has("lantern"))) return;
    const extended = ending === "extended";
    const recorded = ending === "recorded";
    const title = recorded ? "柳外留名" : extended ? "请求展期" : "代林舟还愿";
    const summary = recorded ? "三份柳外记录已经互相校验，四段还记得他的地方也都找回来了。你不能让林舟回来，但可以让论坛与账簿不再垄断他是否存在。" : extended ? "最后一段记忆暂不交割。林舟的名字会留下，他仍不能回来，旧债也不会减少。" : "你将交出关于林舟的全部童年记忆。他会从你的人生中彻底消失，而你会过得很好。";
  ...
  document.addEventListener("click", (event) => {
    ...
    switch (action) {
      ...
      case "open-resource": {
        ...
        if (resource === "news" && !has("keeper")) { toast("还不知道守柳人柜子里那册剪报的存在。先看他的第一笔账。"); break; }
        if (resource === "mail" && !has("codex")) { toast("还不知道林舟在柳外留过名字的地方。先补全古籍异文。"); break; }
        if (resource === "map" && !has("accident")) { toast("剪报还没读完。先去电子报核对那则短讯，再决定看哪一片地图。"); break; }
      ...
      case "restore-diary":
        ...
        else { state.diaryOrder = []; diaryMessage = "顺序无法重组。先分清哪一页是回家之前、哪一页是回家当晚、哪一页是许愿之后。"; save(); render(); }
        break;
      case "keep-diary":
      ...
      case "open-news":
        if (!has("keeper")) { toast("还不知道守柳人柜子里那册剪报的存在。先看他的第一笔账。"); break; }
        navigate("news"); break;
      case "read-news": {
        const clipKey = target.dataset.clip;
        if (["short", "busco", "custom"].includes(clipKey) && !state.newsReads.includes(clipKey)) state.newsReads.push(clipKey);
        save(); render(); break;
      }
      case "map-layer":
        if (["admin", "busline", "found"].includes(target.dataset.layer)) state.mapLayer = target.dataset.layer;
        save(); render(); break;
      case "keep-lantern":
        if (has("lantern") || !has("homeDate")) break;
        collect("lantern"); render(); break;
  ...
  document.addEventListener("submit", (event) => {
    ...
    if (form.id === "news-audit-form") {
      if (!["short", "busco", "custom"].every((key) => state.newsReads.includes(key))) { $("#news-error").textContent = "三张剪报还没看完。"; return; }
      if ($("#news-date").value !== "0617" || $("#news-time").value !== "2340" || $("#news-place").value !== "bank") { $("#news-error").textContent = "对不上。短讯的日期、时刻和地点，再回去读一遍。"; return; }
      collect("accident"); render(); return;
    }
    if (form.id === "mail-form") {
      const card = $("#mail-card").value.trim().toUpperCase();
      if (card !== "LX0617") { $("#mail-error").textContent = "读者证号不符。凭单与读者证同号——他走到哪里，都是同一个编号。"; return; }
      collect("borrow"); render(); return;
    }
    if (form.id === "map-form") {
      if ($("#map-place").value !== "east") { $("#map-error").textContent = "还没对上。从北门桥走向河堤，中间会经过哪一段？去「拾遗登记」图层里找。"; return; }
      collect("eastGate"); render(); return;
    }
